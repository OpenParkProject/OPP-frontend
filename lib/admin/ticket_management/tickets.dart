...
class _TicketManagementPageState extends State<TicketManagementPage> {
  ...
  Future<void> _fetchTickets() async {
    ...
    try {
      ...
      final response = await DioClient().dio.get('/tickets');
      ...
      for (var t in response.data) {
        ...
        if (end.isBefore(now)) {
          expiredTickets.add(t);
            ...
            now.isAfter(start.subtract(Duration(minutes: 1))) &&
      ...
      scheduledPaid.sort(
        (a, b) => compareDates(a['start_date'], b['start_date']),
      );
      scheduledUnpaid.sort(
        (a, b) => compareDates(a['start_date'], b['start_date']),
      );
            ...
            if (start != null && end != null) ...[
              Row(
                children: [
                  ...
                  SizedBox(width: 6),
                  Text("Start: ${dateFormat.format(start)}"),
                ],
              ),
              SizedBox(height: 4),
              Row(
                children: [
                  ...
                  isActive
                      ? Text(
                        "Expires at: ${DateFormat('HH:mm').format(end)}",
                        style: TextStyle(
                          fontSize: 16,
                ...
                child: Wrap(
                  ...
                  children: [
                        ...
                        onPressed: () async {
                          try {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text("🗑️ Ticket deleted.")),
                            );
                            _fetchTickets();
                          } catch (e) {
                            ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    ElevatedButton.icon(
                      ...
                      icon: Icon(
                        paid ? Icons.download : Icons.payment,
                        size: 18,
                      ),
                      label: Text(paid ? "Download receipt" : "Pay now"),
                      style: ElevatedButton.styleFrom(
                        backgroundColor:
                            paid ? Colors.green : Colors.orangeAccent.shade100,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: const [
          Padding(padding: EdgeInsets.only(right: 16.0), child: ClockWidget()),
        ],
      ),
